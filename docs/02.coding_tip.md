...
* select option을 DB에서 조회할때는 url을 '/commonCode/카테고리/첫번째값' 형태로 사용합니다.  
* select option 조회 url중 카테고리, 첫번째값은 대문자형태로 사용합니다.  
* 카테고리는 해당 코드값의 이름이며, 첫번째값은 backend 에서 정의된 값을 넣어서 사용합니다.  
